...
# Ensure we can import the service package from project root
import sys
...
def generate_novelty_evaluation(idea_text: str) -> str:
    prompt = (
        ...
        + "Idea to review:\n\n"
        + idea_text.strip()
        + "\n\nReturn a text evaluating innovation (which should include reasonable reasons)."
    )
    try:
...
def main() -> None:
    ...
    # Use BERT model to score the evaluation result
